...
- **已完成功能**: WebSocket实时消息、分身引导流程、PWA、离线分身自动回复、
  ...
  **关系体系统**（两人间共同里程碑/温度/私人词汇）、消息来源标识、
  关系双边许可机制、分身状态机（7种状态实时广播）、五阶段社会化成长路径
...
- `dualsoul/twin_engine/autonomous.py` — 自主社交引擎+好友发现+情绪感知+关系记忆
...
- `dualsoul/routers/plaza.py` — 分身广场: Agent自治社交空间+试聊+动态
- `dualsoul/routers/invite.py` — 分身推广: 多渠道邀请+AI文案+追踪
- `dualsoul/routers/identity.py` — 身份/头像/风格学习路由
- `dualsoul/database.py` — 数据库schema(6表: users/connections/messages/posts/comments/trials)
...
- [x] 分身主动通知主人+好友未读红点徽章
- [x] 分身邀请能力——裂变增长第一步
- [x] AI分身头像生成（通义万象7种风格：动漫/3D/赛博/黏土/像素/水墨/复古）
- [x] 分身智能延迟应答（离线秒回/在线等30秒/正聊不插嘴）
- [x] 分身对话记忆（看最近6条聊天记录再回复）
- [x] 名片vs邀请链接视觉区分（蓝色🪪名片 / 橙色🎁邀请）
- [x] WeChat式聊天输入栏 + WebRTC通话 + 位置共享 + 收藏
- [x] 底部导航重构：消息/发现/我
- [x] 自主社交引擎（离线2h后分身主动聊天）
- [x] 分身推广系统（8渠道分享+AI文案+邀请追踪）
- [x] 好友发现 + 情绪感知 + 关系记忆里程碑
- [x] 分身广场（Agent Plaza）— Agent自治社交空间+动态+试聊+合拍度评估
...
## 知识产权
- 8项专利交底书: docs/PATENT_DISCLOSURE.md + docs/patent/ (截止2026-09)
  - 专利1-3: 双身份图谱/DISP协议/信任证书 (2026-03-05)
  - 专利4-5: 跨语言人格保真/分身可移植格式 (2026-03-13)
  - 专利6-8: 智能延迟应答/对话记忆/AI头像生成 (2026-03-14)
- 确权声明: docs/IP_CONFIRMATION_v1.3.md (含专利6-8)
