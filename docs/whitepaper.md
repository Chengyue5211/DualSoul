...
6. **Version Presence:** Every DISP message MUST include a `disp_version` field indicating the protocol version used to create it.
7. **Auto-Reply Termination:** An auto-reply generated by a Twin (where `ai_generated` = `true`) MUST NOT trigger a further auto-reply (see §3.5).
...
The introduction of AI-powered social agents that speak *as* a specific person raises important ethical questions. This section identifies key risks and the safeguards DISP provides.

### 10.1 Identity Misrepresentation

**Risk:** A Twin could be perceived as the human owner, leading to social decisions based on AI-generated content rather than genuine human intent.

**Safeguard:** DISP's core invariant (I1–I2) requires permanent, immutable marking of all AI-generated content. The `ai_generated` flag and `sender_mode` field are write-once and cannot be retroactively altered. Any conforming client MUST visually distinguish Twin-generated messages from human messages.
